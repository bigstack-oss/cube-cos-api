...
func (h *helper) syncCreatingFiles(files *[]support.File) {
	...
	if err != nil {
		log.Errorf("supportFiles(%s): failed to get creating file set: %s", h.reqId, err.Error())
		return
	}
...
func (h *helper) setCreatingFile(files *[]support.File, c *mongo.Cursor) {
	...
	for c.Next(ctx) {
		...
		err := c.Decode(&file)
		if err != nil {
			log.Errorf("supportFiles(%s): failed to decode creating file set: %s", h.reqId, err.Error())
			continue
		}
