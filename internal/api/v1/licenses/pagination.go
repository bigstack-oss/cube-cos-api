...
func paginateLicenses(licenses []definition.License, page definition.Page) ([]definition.License, error) {
	if !page.IsRequired() {
		return licenses, nil
	}

	left := (page.Number - 1) * page.Size
	if left > len(licenses) {
		left = len(licenses)
	}

	right := left + page.Size
	if right > len(licenses) {
		right = len(licenses)
	}

	return licenses[left:right], nil
}
